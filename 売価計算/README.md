...
本件の原因は、**Kintone のサブテーブル DOM 再描画タイミングと MutationObserver の発火順序** によるものでした。
...
- **備考欄送信機能の追加**
  売価計算アプリから掲載依頼アプリへのデータ転送処理において、各媒体の備考フィールドを送信対象に追加。
  `noteFieldMap` を整理し、重複していた処理を統一。

### 🧮 対応内容詳細

| モール名             | 売価計算側       | 掲載依頼側       | 備考           |
| -------------------- | ---------------- | ---------------- | -------------- |
| くまポン             | 備考\_kumapon    | 備考\_くまポン   | 既存維持       |
| リロクラブ           | 備考\_リロ       | 備考\_リロ       | 新設           |
| ベネフィットワン     | 備考\_ベネ       | 備考\_ベネ       | 新設           |
| KAUCHE               | 備考\_おためし   | 備考\_おためし   | 新設           |
| V 景品交換           | 備考\_T ポイント | 備考\_T ポイント | 新設           |
| 社内販売ネットモール | 備考\_社販       | 備考\_社販       | 新設           |
| TEMU                 | 備考\_TEMU       | 備考\_temu       | 小文字変換対応 |
| BEAUTH               | 備考\_eecoto     | 備考\_eecoto     | 既存維持       |
| モラタメ             | 備考\_モラタメ   | 備考\_モラタメ   | 既存維持       |
| WELBOX               | 備考\_FiNC       | 備考\_FiNC       | 既存維持       |

### 🧠 技術的補足

- `SetReqRecord()` 内で備考欄送信処理を実装。
- 掲載依頼アプリ側フィールドコードに統一し、モール名別の重複ロジックを削除。
- TEMU のみフィールドコードが小文字のため変換処理を追加。

### 🧾 確認結果

- 各モールで掲載依頼アプリに備考が正常に転送されることを確認。
- 既存の採用情報・価格連携・ステータス更新処理に影響なし。

### 🪣 Git 反映メモ

```
git add 売価計算/01_1_CreateRecodeToPostingRequest_250703.js
git commit -m "feat(掲載依頼連携): 備考欄送信機能を全媒体対応に追加"
git push origin main
```
